...
**Styling & motion (when relevant):** `brand-designer/`, `tailwindcss/`, `tailwind-design-system/`, `tailwind-css-patterns/`, `tailwindcss-advanced-layouts/`, `tailwindcss-animations/`, `ui-animation/` — each under `.cursor/skills/<name>/` (Cursor) or `.claude/skills/<name>/` (Claude Code). Full grouped list: `.cursor/agents/cook.md` → **Skills catalog**.
