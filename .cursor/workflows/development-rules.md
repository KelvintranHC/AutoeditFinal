...
## Greenfield frontends

When creating a **new** React/Next/Vite (etc.) UI codebase: follow the stack and plan the user or `package.json` implies. Read **`web-frameworks`**, **`frontend-development`**, and **`ui-styling`** skills only as needed. For Tailwind-heavy or design-system work, also pull from the **Tailwind & utility CSS** and **Brand & motion** rows in `.cursor/agents/cook.md` (**Skills catalog**). Do **not** add a prescribed component library or setup CLI unless the user explicitly requests it. Cook aligns with **New apps & UI work** in `.cursor/agents/cook.md`.