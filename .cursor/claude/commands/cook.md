...
**IMPORTANT:** Analyze skills under **`.cursor/skills/*`** and **`.cursor/claude/skills/*`** (full catalog) and intelligently read only the skills needed for the task during the process.

### Skills catalog (full repo inventory)

- **Canonical grouped table:** `.cursor/agents/cook.md` → **Skills catalog** (maintain this when adding skills).
- **On disk in this repo:** **`.cursor/skills/<name>/SKILL.md`** (primary Cursor tree) and **`.cursor/claude/skills/<name>/SKILL.md`** (Claude Code bundle + merged skills). Some names exist in both trees; prefer the path that exists for the skill you need.
- **Authoritative list:** glob **`.cursor/skills/**/SKILL.md`** and **`.cursor/claude/skills/**/SKILL.md`** (includes nested folders like `document-skills/docx/`).
- **Styling suite (Tailwind + motion + brand):** `brand-designer/`, `tailwindcss/`, `tailwind-design-system/`, `tailwind-css-patterns/`, `tailwindcss-advanced-layouts/`, `tailwindcss-animations/`, `ui-animation/`.
