...
claude-legacy: .cursor/claude/commands/review/codebase.md
cursor-agent: .cursor/agents/code-reviewer.md
---

# Cursor command — `/review codebase`

## Cursor parity (no Task spawn)

- **Primary persona:** Open and follow `.cursor/agents/code-reviewer.md` for this command.
...
* Use 2 **researcher** persona (see `.cursor/agents/researcher.md`) in parallel to search up to max 5 sources for the user's request, idea validation, best practices, challenges, and find the best possible solutions.
...
### Plan
* Use **planner** persona (`.cursor/agents/planner.md`) to analyze reports from `researcher` and **scout** persona (see `.cursor/agents/scout.md`) to create an improvement plan following the progressive disclosure structure:
